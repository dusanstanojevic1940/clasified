...
public class FeaturedAd {
	...
	public String getPricePerDay() {
		return settingsDAO.getSettings().priceFeaturedAdCoinsPerDay+"";
	}
    ...
    public Object makeTop() {
		...
		Date latestDate = postDAO.getLatestDateForFeaturedAd(post_id)==null?new Date():postDAO.getLatestDateForFeaturedAd(post_id);
		
		if (c*settingsDAO.getSettings().priceFeaturedAdCoinsPerDay>user.coin) {
			//ajaxResponseRenderer.addCallback(new JavaScriptCallback() {
				
				//@Override
				//public void run(JavaScriptSupport jss) {
					//jss.addScript("alert(\"You do not have enough coins. You should buy more coins\");");
			...
			//for (int i = 1; i<=c+1; i++)
				//postDAO.addFeaturedPostOn(post_id, DateUtils.addDays(latestDate, i));
			postDAO.addFeaturedPostOn(post_id, latestDate, DateUtils.addDays(latestDate, c));
			user.coin-=c*settingsDAO.getSettings().priceFeaturedAdCoinsPerDay;
			userDAO.update(user);
